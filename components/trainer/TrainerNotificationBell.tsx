...
import { countUnread, trainerNotificationHref } from "@/lib/domain/notifications";
import { formatDateTime } from "@/lib/format";

const POLL_MS = 20000;

/**
 * Bell icon rendered near the trainer sidebar's user menu — the trainer-side
 * counterpart to AdminNotificationBell, tracking this trainer's own
 * notifications (new bookings, submission/correction/cancellation
 * decisions). Polls on an interval (this app has no push/websocket layer)
 * so the unread count stays current even without a manual refresh.
 */
export function TrainerNotificationBell({
  ...
  async function handleSelect(notification: Notification) {
    ...
    if (!notification.read) {
      await fetch(`/api/trainer/notifications/${notification.id}`, {
        method: "PATCH",
      });
    }
    router.push(trainerNotificationHref(notification));
  }

...
/** Central review queue — every trainer's currently "submitted" weeks, with Approve/Reject actions. Rejecting requires a reason. */
export function SubmissionsList({
  initialSubmissions,
}: {
  initialSubmissions: EnrichedWeeklySubmission[];
}) {
  const [submissions, setSubmissions] = useState(initialSubmissions);

  async function refresh() {
    const response = await fetch("/api/admin/weekly-submissions");
...
function SubmissionCard({
  ...
  submission: EnrichedWeeklySubmission;
  onDecided: () => void;
}) {
  ...
  async function decide(status: "approved" | "rejected") {
    ...
    try {
      const response = await fetch(
        `/api/admin/weekly-submissions/${submission.id}`,
        {
          method: "PATCH",
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-brand-darkBlue/50">
            {formatHours(submission.totalHours)} logged
          </p>

...
export async function GET() {
  ...
  const submissions = await store.listTimesheetSubmissionsForTrainer(
    trainerId,
    { id: trainerId, role: auth.session.role }
  );
...
export async function POST(request: Request) {
  ...
  try {
    const submission = await store.submitTimesheetWeek(weekStartDate, {
